...
app.get("/api/videos", async (_req, res) => {
  ...
  res.json({
    videos: db.videos.map((video) => ({
      ...
      title: video.title || "Scoutify upload",
...
app.get("/api/auth/google/callback", async (req, res) => {
  ...
  if (!client || !req.query.code) {
    return res.redirect("/?authError=Google%20sign-in%20is%20not%20configured");
  }
  ...
  try {
    ...
    res.redirect(`/?authError=${encodeURIComponent(error.message || "Google sign-in failed")}`);
  }
});
...
app.post("/api/videos/youtube", auth, upload.single("video"), async (req, res) => {
  ...
  if (!client || !user?.googleTokens) {
    return res.status(400).json({ error: "Connect Google first to upload to YouTube." });
  }
  if (!req.file) {
  ...
  try {
    ...
    const result = await youtube.videos.insert({
      ...
      requestBody: {
        snippet: {
          title: req.body.title || "Scoutify upload",
          description: req.body.description || "Uploaded from Scoutify"
        },
        status: {
      ...
      media: {
        ...
        body: stream.createReadStream(req.file.path)
      }
    });

    const video = {
      id: randomUUID(),
      youtubeId: result.data.id,
      title: req.body.title || "Scoutify upload",
      owner: user.id,
      createdAt: new Date().toISOString()
    };
