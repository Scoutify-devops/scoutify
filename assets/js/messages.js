...
function renderConversations(filter = "") {
  ...
  document.getElementById("conversationList").innerHTML = conversations.filter((chat) => `${chat.name} ${chat.preview}`.toLowerCase().includes(query)).map((chat) => `<button class="conversation ${chat.id === activeId ? "active" : ""}" data-chat="${chat.id}"><span class="dm-avatar ${chat.color}">${chat.avatar}</span><span class="conversation-copy"><strong>${chat.name}</strong><small>${chat.preview}</small></span><time>${chat.time}</time></button>`).join("") || "<p class='empty-state'>No conversations found.</p>";
...
function renderChat() {
  ...
  const messages = readMessages()[activeId] || [];
